...
export const amazonUkScript: ExtractionScript = {
  ...
  getNavigationTargets(keyword: string, _market: MarketContext): string[] {
    return [
      `https://www.amazon.co.uk/s?k=${encodeURIComponent(keyword)}`,
  ...
  extractFromPage(document: Document, url: string): RawPlatformData | null {
    ...
    if (url.includes('amazon.co.uk/s?') || url.includes('amazon.co.uk/s/')) {
    ...
    if (url.includes('amazon.co.uk/Best-Sellers') || url.includes('zgbs')) {
...
registry.register(amazonUkScript);
