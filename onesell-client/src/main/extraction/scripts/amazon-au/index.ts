...
export const amazonAuScript: ExtractionScript = {
  platformId: 'amazon-au',
  marketId: 'au',
  version: '1.0.0',
  homeUrl: 'https://www.amazon.com.au',

  getNavigationTargets(keyword: string, _market: MarketContext): string[] {
    return [
      `https://www.amazon.com.au/s?k=${encodeURIComponent(keyword)}`,
      'https://www.amazon.com.au/gp/bestseller',
    ];
  },
  ...
  extractFromPage(document: Document, url: string): RawPlatformData | null {
    ...
    if (url.includes('amazon.com.au/s?') || url.includes('amazon.com.au/s/')) {
    ...
    if (url.includes('amazon.com.au/gp/bestseller') || url.includes('zgbs')) {
...
registry.register(amazonAuScript);
