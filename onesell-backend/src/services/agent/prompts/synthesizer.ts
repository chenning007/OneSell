 ...
 *
 * Role: Take the Executor's tool outputs + reasoning and produce the final
 * ranked product shortlist as ProductCard[]-compatible output.
 *
 * Principles enforced:
 *  - P3: All numbers cited must trace back to tool outputs
 *  - P4: MarketContext determines output language, currency, platform names
 *  - P5: Handle partial results gracefully — fewer cards, not failure
 *  - P8: Market-specific content from MarketContext
 *  - P9: Output validation, anti-hallucination, injection defence
 */
...
// ── Market-specific synthesis guidance ──────────────────────────────
