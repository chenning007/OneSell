 ...
 * Fee data lives in config objects, not hardcoded in logic (P8).
 *
 * Closes #120
 */
...
/**
 * Platform fee configuration (P8: data in config, not in logic).
 * Keyed by platformId. All monetary values are in USD-equivalent;
 * currency conversion happens at output using market.currency.
 */
interface PlatformFeeConfig {
  readonly commissionPercent: number;       // e.g. 0.15 = 15%
  readonly listingFee: number;              // per-listing fee in USD
  readonly paymentProcessingPercent: number; // e.g. 0.029 = 2.9%
